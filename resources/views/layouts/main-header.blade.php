<nav class="admin-header navbar navbar-expand-lg navbar-light bg-light fixed-top d-flex flex-row">
    <!-- logo -->
    ...
    <ul class="nav navbar-nav ml-auto">
        <li>
            <select name="language" id="language-select" class="btn btn-dark" onchange="window.location.href=this.value;">
                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                    <option value="{{ LaravelLocalization::getLocalizedURL($localeCode) }}" {{ LaravelLocalization::getCurrentLocale() == $localeCode ? 'selected' : '' }}>
                        {{ $properties['native'] }}
                    </option>
                @endforeach
            </select>
